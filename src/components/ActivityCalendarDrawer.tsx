...
const ProgressRings = ({ stepProgress, calProgress }: { stepProgress: number, calProgress: number }) => {
  ...
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" className="transform -rotate-90">
      {/* Background Rings */}
      <circle cx="12" cy="12" r={stepR} fill="none" stroke="hsl(var(--chart-steps) / 0.15)" strokeWidth="2.5" />
      <circle cx="12" cy="12" r={calR} fill="none" stroke="hsl(var(--primary) / 0.15)" strokeWidth="2.5" />

      {/* Progress Rings */}
      {stepProgress > 0 && (
        <circle cx="12" cy="12" r={stepR} fill="none" stroke="hsl(var(--chart-steps))" strokeWidth="2.5" strokeLinecap="round" strokeDasharray={stepDash} />
      )}
      {calProgress > 0 && (
        <circle cx="12" cy="12" r={calR} fill="none" stroke="hsl(var(--primary))" strokeWidth="2.5" strokeLinecap="round" strokeDasharray={calDash} />
      )}
    </svg>
  );
};
...
const ActivityCalendarDrawer: React.FC<ActivityCalendarDrawerProps> = ({
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="h-[85vh] elevation-glass border-t rounded-t-[20px]">
        <div className="mx-auto w-12 h-1.5 flex-shrink-0 rounded-full bg-muted mt-4 mb-2" />
        <DrawerHeader className="text-left pb-2">
          <div className="flex justify-between items-center w-full px-2">
            <DrawerTitle className="font-display text-2xl font-semibold tracking-tight">{year}</DrawerTitle>
        ...
        <div className="px-6 py-4 overflow-y-auto pb-10">
          <div className="mb-6">
            <h2 className="font-display text-2xl font-semibold tracking-tight mb-4">
              {monthName}{' '}
              <span className="font-sans text-muted-foreground text-sm font-normal ml-2">
                All goals reached: 0/30
              </span>
            </h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div>
                <p className="text-sm text-muted-foreground flex items-center gap-2 mb-1">
                  <span className="w-1.5 h-4 rounded-full bg-chart-steps inline-block"></span>
                  Avg. steps
                </p>
                <p className="font-mono font-bold text-lg tabular-nums">
                  {avgSteps.toLocaleString()} <span className="font-sans text-sm font-normal">steps</span>
                </p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground flex items-center gap-2 mb-1">
                ...
                <p className="font-mono font-bold text-lg tabular-nums">
                  {avgCalories.toLocaleString()} <span className="font-sans text-sm font-normal">kcal</span>
                </p>
              </div>
            </div>
              ...
              {calendarDays.map((date, index) => {
                ...
                return (
                  <div key={date.toISOString()} className="flex flex-col items-center justify-start h-16">
                    <span
                      className={`font-mono text-sm mb-2 w-7 h-7 flex items-center justify-center tabular-nums ${
                        isToday ? 'bg-foreground text-background font-bold rounded-full' : ''
                      }`}
                    >
                      {date.getDate()}
                    </span>
